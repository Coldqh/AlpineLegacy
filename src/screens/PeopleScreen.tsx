  ...
  return (
    <section className="workspace-page people-page">
      <header className="workspace-title workspace-title--compact">
        ...
        <div className="workspace-title__mark"><span>{career.teamRoster.filter(member => member.status === 'ACTIVE').length}</span><small>АКТИВНЫХ</small></div>
      </header>
      ...
      <div className="people-layout">
          ...
          <div className="people-list">
            {career.teamRoster.map(member => (
              <button key={member.id} className={selected.id === member.id ? 'is-active' : ''} onClick={() => setSelectedId(member.id)}>
                ...
                <span><small>{roleLabel[member.role]} · {statusLabel[member.status]}</small><strong>{member.name}</strong><em>{relationName(member)}</em></span>
        ...
        <section className="person-file workspace-panel">
          <div className="person-file__hero">
            ...
            <div><p className="eyebrow">PERSON FILE / {selected.id.toUpperCase()}</p><h2>{selected.name}</h2><p>{selected.age} лет · {roleLabel[selected.role]} · {SKILL_LABELS[selected.specialty]} {selected.skill}/10</p></div>
