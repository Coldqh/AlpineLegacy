  ...
  return (
    <main className="m-public-shell">
      ...
      <section className="m-wizard">
        ...
        {step === 2 && <><h1>{name}</h1><div className="m-review-card"><div><span>Возраст</span><strong>{age}</strong></div><div><span>Школа</span><strong>{origin.title}</strong></div><div><span>Деньги</span><strong>{origin.startingMoney}</strong></div><div><span>Форма</span><strong>{origin.startingForm}</strong></div></div><details className="m-details"><summary>Навыки</summary><div className="m-skill-list">{Object.entries(origin.skills).map(([id, value]) => <div key={id}><span>{SKILL_LABELS[id as keyof typeof SKILL_LABELS]}</span><strong>{value}/10</strong><i><b style={{ width: `${Number(value) * 10}%` }} /></i></div>)}</div></details></>}
      </section>
