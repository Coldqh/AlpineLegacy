  ...
  if (!climb || !topo) {
    return (
      <main className="mg-app">
        <header className="mg-header"><div><span>ALPINE LEGACY / 0.26.0</span><h1>Экспедиция недоступна</h1></div><div className="mg-header-actions"><button onClick={() => onExit(true)}>Вернуться</button></div></header>
      </main>
    );
  }
    ...
    <main className={`mg-app mg-expedition-shell is-${clock.phase} is-weather-${weatherMood} is-fx-${fieldFx.toLowerCase()}`}>
      <header className="mg-header mg-expedition-header">
        <div className="mg-header-copy">
          <span>ALPINE LEGACY / 0.26.0</span>
