...
## 0.7.1 — 3D Mountain Grid Prototype

- свободное рисование маршрута удалено из активного прототипа;
- гора генерируется как единое высотное поле и показывается интерактивной 3D-моделью;
- модель можно вращать, наклонять и приближать мышью или пальцем;
- та же гора разворачивается в квадратную карту 21 × 21;
- каждая клетка содержит высоту, уклон, тип рельефа, опасность и пригодность для лагеря;
- руководитель прокладывает путь по соседним клеткам вручную;
- участник получает готовый маршрут NPC и не редактирует общий план;
- группа автоматически проходит всю запланированную цепочку этапов;
- маршрут можно продолжить, сократить или изменить на активной паузе;
- добавлены управление ведущим, порядком трёх участников, связкой и дистанцией;
- лагеря и закреплённые верёвки размещаются на конкретных клетках и остаются на спуск;
- скрытые трещины, лавинный снег и камнепад физически привязаны к клеткам;
- погода меняет скорость, видимость и состояние снега;
- вершина переводит игру в отдельную фазу спуска;
- добавлены тесты детерминированной генерации, связности маршрута, лагерей и времени прохождения.
